...
<html>
<head>
    <title>Edit Staff - PKU Management System</title>
    ...
    <div class="dashboard-wrapper">
        ...
        <main class="main-content">
                ...
                <div class="form-container">
                    <h2 class="section-heading">Edit Staff</h2>
                    <form action="StaffServlet" method="post" onsubmit="return validateForm()">
                        <input type="hidden" name="action" value="edit">

                        <div class="form-group">
                            <label for="userID">User ID:</label>
                            <input type="text" id="userID" name="userID" class="form-control" value="<%= staff.getUserID() %>" readonly>
                        </div>

                        <div class="form-group">
                            <label for="icnum">IC Number:</label>
                            <input type="text" id="icnum" name="icnum" class="form-control" value="<%= staff.getIcnum() %>" required>
                        </div>

                        <div class="form-group">
                            <label for="password">Password:</label>
                            <input type="password" id="password" name="password" class="form-control" value="<%= staff.getPassword() %>" required>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="role" id="role" class="form-control" onchange="showRoleFields()" required>
                                ...
                                <option value="admin" <%= staff.getRole().equals("admin") ? "selected" : "" %>>Admin</option>
                                <option value="doctor" <%= staff.getRole().equals("doctor") ? "selected" : "" %>>Doctor</option>
                                <option value="receptionist" <%= staff.getRole().equals("receptionist") ? "selected" : "" %>>Receptionist</option>
                            </select>
                        </div>

                        <div class="form-group">
                            <label for="name">Name:</label>
                            <input type="text" id="name" name="name" class="form-control" value="<%= staff.getName() %>" required>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="phoneNumber" name="phoneNumber" class="form-control" value="<%= staff.getPhoneNumber() %>" required>
                        </div>

                        <div class="form-group">
                            <label for="status">Status:</label>
                            <select name="status" id="status" class="form-control" required>
                                <option value="A" <%= staff.getStatus() == 'A' ? "selected" : "" %>>Active</option>
                                <option value="B" <%= staff.getStatus() == 'B' ? "selected" : "" %>>Inactive</option>
                            </select>
                        </div>
                        ...
                        <div id="receptionistFields" class="role-field">
                            <div class="form-group">
                                ...
                                <input type="number" id="desk_no" name="desk_no" class="form-control" 
                                       value="<%= request.getAttribute("desk_no") != null ? request.getAttribute("desk_no") : "" %>">
                            </div>
                        </div>
                            <br>
                ...
                <% } else { %>
                    <p class="message error-message">Staff not found.</p>
                    <a href="StaffServlet" class="back-link">
                        <i class="fas fa-arrow-left"></i> Back to Staff List
                    </a>
                <% } %>
